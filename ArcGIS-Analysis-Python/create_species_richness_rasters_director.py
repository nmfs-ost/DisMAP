...
def director(project_gdb="", Sequential=True, table_names=[]):
    try:
        ...
        del create_species_richness_rasters_worker
        ...
        if not table_names:
            table_names = [row[0] for row in arcpy.da.SearchCursor("Datasets", "TableName", where_clause = f"GeographicArea IS NOT NULL and SummaryProduct = 'Yes'")]
        ...
        for table_name in table_names:
            ...
            datasets = [rf"{project_gdb}\Datasets", rf"{project_gdb}\{table_name}_LayerSpeciesYearImageName", rf"{project_gdb}\{table_name}_Raster_Mask"]
...
def main(project="", test=False):
    try:
        ...
        try:
            ...
            if test:
                result = director(project_gdb=project_gdb, Sequential=True, table_names=["AI_IDW"])
...
##                results.extend(result); del result
            ...
            # NOTE: DON'T RUN THIS FOR "WC_GFDL", "WC_GLMME"
...
if __name__ == '__main__':
    try:
        ...
        import create_species_richness_rasters_director
        importlib.reload(create_species_richness_rasters_director)
