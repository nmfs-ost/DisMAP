...
def director(project_gdb="", Sequential=True, table_names=[]):
    try:
        ...
        del create_indicators_table_worker
        ...
        for table_name in table_names:
            ...
            indicators = os.path.join(project_gdb, f"{table_name}_Indicators")

            region_gdb = rf"{scratch_folder}\{table_name}.gdb"

            region_indicators = os.path.join(region_gdb, f"{table_name}_Indicators")
        ...
        if not Sequential and not results:
...
def process_indicator_tables(project_gdb="", project=""):
    try:
        # Import
        import dismap
        importlib.reload(dismap)
        ...
        dismap.add_fields(csv_data_folder, indicators)
        dismap.alter_fields(csv_data_folder, indicators)
        dismap.import_metadata(indicators)
...
def main(project="", test=False):
    try:
        # Import
        from create_indicators_table_director import director
        ...
        try:
            ...
            if test:
                director(project_gdb=project_gdb, Sequential=True, table_names=["AI_IDW",])
            else:

##                result = director(project_gdb=project_gdb, Sequential=False, table_names=["NBS_IDW", "ENBS_IDW", "GOA_IDW",])
...
##                results.extend(result); del result
##
##                result = director(project_gdb=project_gdb, Sequential=False, table_names=["GMEX_IDW", "AI_IDW", "EBS_IDW",])
##                results.extend(result); del result

                result = director(project_gdb=project_gdb, Sequential=False, table_names=["NBS_IDW", "ENBS_IDW",])
                results.extend(result); del result

                result = director(project_gdb=project_gdb, Sequential=False, table_names=["HI_IDW", "SEUS_FAL_IDW",])
                results.extend(result); del result

                result = director(project_gdb=project_gdb, Sequential=False, table_names=["SEUS_SPR_IDW", "SEUS_SUM_IDW",])
                results.extend(result); del result

                result = director(project_gdb=project_gdb, Sequential=False, table_names=["WC_TRI_IDW", "GMEX_IDW",])
                results.extend(result); del result

                result = director(project_gdb=project_gdb, Sequential=False, table_names=["AI_IDW", "GOA_IDW",])
                results.extend(result); del result

                result = director(project_gdb=project_gdb, Sequential=False, table_names=["WC_ANN_IDW", "NEUS_FAL_IDW",])
                results.extend(result); del result
                ...
                # Not yet
                #result = director(project_gdb=project_gdb, Sequential=False, table_names=["WC_GFDL", "WC_GLMME",])
                #results.extend(result); del result

            # Combine Indicator Tables
            results = process_indicator_tables(project_gdb=project_gdb, project=project)
            del results
            results = []
...
if __name__ == '__main__':
    try:
        ...
        import create_indicators_table_director
        importlib.reload(create_indicators_table_director)
