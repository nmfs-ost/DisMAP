...
flag_spp <- function(data, SurveyName){
  
  cat("taxonomy check for region: ", SurveyName, sep="\n")
  ...
  if (dim(testdf)[1] > 0) {
    ...
    write.csv(data_out, file = here::here("data_processing_rcode", "output", paste0(region, "_stats.csv")), row.names = FALSE)
    ...
    pres <- subset(df_region, Species %in% testdf$spp) %>%
      dplyr::select(Species, Year)
...
flag_spp(data, SurveyName = "Aleutian Islands Bottom Trawl Survey")
