...
summary_tab <- plot_data_spring %>%
  distinct(FishRegion, trend_status) %>%
  mutate(Trend = case_when(
    trend_status == "Significant Increase" ~ "↑ Inc",
    trend_status == "Significant Decrease" ~ "↓ Dec",
    TRUE ~ "— Stable"
  )) %>%
  select(Jurisdiction = FishRegion, Trend)
...
```{r}
#Looking at a stacked barplot of proportion of biomass in each FMC
#Create the Stacked Barplot ---
# Choose a color palette for your 3 jurisdictions
color_palette <- scale_fill_manual(
  name = "Jurisdiction", # Legend title
  values = c("#275F6E","#3B8EA5","#D1E3F0") # Example colors: Blue, Dark Blue, Off-White
)
...
selected_spp <- c("Dipturus laevis", "Rhizoprionodon terraenovae",
...
ggplot(plot_data_spring, aes(x = year, y = prop, fill = FishRegion)) +
    # Stacked bars
  geom_bar(stat = "identity", position = "stack", width = 0.8) +
    # Color palette and legend
  color_palette +
    # Faceting by species
  facet_wrap(~ spp, ncol = 2, scales = "free_x") + # Use 'free_x' if years vary
    # Axis and title formatting
  scale_y_continuous(labels = scales::label_percent()) +
  labs(
    title = "Proportion of Biomass by Management Jurisdiction",
    x = "Year",
