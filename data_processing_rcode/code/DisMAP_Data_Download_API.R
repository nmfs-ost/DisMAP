...
# for the indicators table: https://services2.arcgis.com/C8EMgrsFcRFL6LrL/arcgis/rest/services/Indicators_CURRENT/FeatureServer
# for the survey data: https://www.fisheries.noaa.gov/inport/item/69743
# for the interpolated biomass layers: https://www.fisheries.noaa.gov/inport/item/70034
...
url$path <- paste(url$path, "Indicators_CURRENT/FeatureServer/1/query", sep = "/")
url$query <- list(where = "CommonName = 'American lobster' AND Datasetcode='NEUS_SPR'",
                  outFields = "*",
                  f = "geojson")
...
#### 2. Downloading INTERPOLATED BIOMASS RASTERS ####
  ...
  # make sure to double check the URLs for the data layer you want by going to the InPort page, clicking on Fish and Invertebrate Interpolated Biomass Distribution
    ...
    SumFl_slices <- "https://maps.fisheries.noaa.gov/image/rest/services/DisMAP/NEUS_SPR_IDW_CURRENT/ImageServer/slices?multidimensionalDefinition=%5B%7B%22variableName%22%3A+%22Paralichthys+dentatus%22%7D%5D&f=pjson"
  ...
  #now to use the exportImage function of ESRI API to export the raster images for Summer Flounder based on the slice #s in df
    ...
    for (i in c(1472:1517)){
        print(paste0("Downloading raster image for slice ", i))

        url<-httr::parse_url("https://maps.fisheries.noaa.gov/image/rest/services/DisMAP/NEUS_SPR_IDW_CURRENT/ImageServer/exportImage")
        ...
        bbox_arg <- list(bbox = paste(    #note that the bounding box will be different for each region!
          min(428349.9584999997),
          min(3888846.4584999997),
          max(1278349.9584999997),
          max(4952846.4585),
          sep = ","
        ))
