...
This document presents various analyses for evaluating species persistence (emergence, consistence persistence, or removal) as a new analysis module on [DisMAP](https://apps-st.fisheries.noaa.gov/dismap/). **Feedback on the usefulness of these analyses and visualizations would be greatly appreciated.**
...
* Categorize survey abundance as being: 
  ...
  + abundant
  + or highly abundant
* Measure cumulative trend in prevalence of a particular species, across all survey years (*in development and not presented here*)
...
In this current draft of the analyses, we present data processing steps as well as four draft heat maps. The heat maps reflect
  1. the frequency of occurrence for a particular species (by year and by survey),
  2. the proportion of occurrence for a particular species (by year and by survey),
...
Here, we remove the species that have zero biomass for an entire survey because these values would skew percentile calculations.
```{r}
...
Then, we bring the zero biomass rows back in after the percentiles are calculated and `rbind` them to the data set with percentiles calculated. Species that have zero biomass for an entire survey will have a percentile value of zero. Any species that has a survey biomass > 0 will have a ranked percentile value, in comparison to all the biomass values in that years survey.
```{r}
...
Please feel free to send any feedback on these figures to Melissa and Claire. Specifically, we would appreciate any comments on which depiction of the data (if any) you feel would be most useful to DisMAP end-users.
...
This "Haul Frequency" figure reflects the number of hauls that each species was present in, across surveyed years for the Northeast Spring Bottom Trawl Survey.
```{r}
library(tidyverse)
library(hrbrthemes)
library(viridis)
...
data_1 <- data_full %>%
  ...
  ungroup() %>% 
  dplyr::select(-Region, -SurveyName, -Common, -WTCPUE, -Percentile, -Bin, -Proportion) %>% 
...
This "Haul Proportion" figure reflects the proportion of hauls that each species was present in (hauls present divided by total hauls in a survey), across surveyed years for the Northeast Spring Bottom Trawl Survey.
```{r}
data_2 <- data_full %>%
  filter(SurveyName == "NEFSC Spring Bottom Trawl") %>% 
  ungroup() %>% 
  dplyr::select(-Region, -SurveyName, -Common, -WTCPUE, -Presence, -Bin, -Percentile) %>% 
...
This "Percentiles" figure reflects the biomass of each species, calculated as a percentile of all the species biomass values across an entire survey. Percentiles are calculated for all survey years for the Northeast Spring Bottom Trawl Survey.
```{r}
data_3 <- data_full %>%
  filter(SurveyName == "NEFSC Spring Bottom Trawl") %>% 
  ungroup() %>% 
  dplyr::select(-Region, -SurveyName, -Common, -WTCPUE, -Presence, -Bin, -Proportion) %>% 
...
This "Haul Frequency" figure reflects the number of hauls that each species was present in, across surveyed years for the Aleutian Islands Bottom Trawl Survey.
```{r}
data_5 <- data_full %>%
...
p5
```
This "Haul Proportion" figure reflects the proportion of hauls that each species was present in (hauls present divided by total hauls in a survey), across surveyed years for the Aleutian Islands Bottom Trawl Survey.
```{r}
data_6 <- data_full %>%
  filter(SurveyName == "Aleutian Islands Bottom Trawl Survey") %>% 
  ungroup() %>% 
...
This "Percentiles" figure reflects the biomass of each species, calculated as a percentile of all the species biomass values across an entire survey. Percentiles are calculated for all survey years for the Aleutian Islands Bottom Trawl Survey.
```{r}
data_7 <- data_full %>%
  filter(SurveyName == "Aleutian Islands Bottom Trawl Survey") %>% 
  ungroup() %>% 
