...
**FLAG: Add common names back into these data frames. We did it for bfish already. So just need to do the others**
...
**Flag to do: create a column of bins here** 
```{r binning}
...
presyr <- present_every_year(data_update, region, survey, spp, year)
...
data_haul <- preshaul %>% 
  select(region, survey, spp, year, pres) %>% 
  dplyr::rename(Region = region,
                Species = spp,
                SurveyName = survey,
                Year = year,
...
```{r}
data_full <- data_rank %>% 
  left_join(data_haul, by = c("Region", "SurveyName", "Species", "Year")) %>% 
...
data_1 <- data_full %>%
  filter(SurveyName == "West Coast Bottom Trawl Triennial") %>% 
  ungroup() %>% 
  dplyr::select(-Region, -SurveyName, -WTCPUE, -Percentile) %>% 
...
p1 <- heatmaply(mat_1,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 1: West Coast Bottom Trawl Triennial",
                # scale = "column",
...
p2 <- heatmaply(mat_2,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 2: NEFSC Spring Bottom Trawl",
                # scale = "column",
                # margins = c(60,100,40,20),
                # grid_color = "white",
                # grid_width = 0.000005,
...
```{r}
data_3 <- data_full %>%
  filter(SurveyName == "Eastern Bering Sea Crab/Groundfish Bottom Trawl Survey") %>% 
  ungroup() %>% 
  dplyr::select(-Region, -SurveyName, -WTCPUE, -Percentile) %>% 
  pivot_wider(names_from = Year, values_from = Presence)
...
p3 <- heatmaply(mat_3,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 3: Eastern Bering Sea Crab/Groundfish Bottom Trawl Survey",
                # scale = "column",
                # margins = c(60,100,40,20),
                # grid_color = "white",
                # grid_width = 0.000005,
...
### Biomass Percentile Heatmaps
```{r}
data_4 <- data_full %>%
  filter(SurveyName == "West Coast Bottom Trawl Triennial") %>% 
  ungroup() %>% 
  dplyr::select(-Region, -SurveyName, -WTCPUE, -Presence) %>% 
  pivot_wider(names_from = Year, values_from = Percentile)

# data_1$Year <- as.character(data_1$Year)

mat_4 <- as.matrix(data_4)
rownames(mat_4) <- mat_4[,1]
mat_4 <- as.data.frame(mat_4) %>% 
  dplyr::select(-Species) %>% 
  mutate_if(is.character, as.numeric)
# mat_1 <- as.matrix(mat_1) %>% 
  # mutate_if(is.character, as.numeric)
...
p4 <- heatmaply(mat_4,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 4: West Coast Bottom Trawl Triennial (Percentiles)",
                # scale = "column",
                # margins = c(60,100,40,20),
                # grid_color = "white",
                # grid_width = 0.000005,
                ...
                fontsize_row = 5, 
                fontsize_col = 5,
                labCol = colnames(mat_4),
                labRow = rownames(mat_4),
                heatmap_layers = theme(axis.line = element_blank())
                )
...
p5 <- heatmaply(mat_5,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 5: NEFSC Spring Bottom Trawl (Percentiles)",
                # scale = "column",
                # margins = c(60,100,40,20),
                # grid_color = "white",
                # grid_width = 0.000005,
...
p6 <- heatmaply(mat_6,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 6: Eastern Bering Sea Crab/Groundfish Bottom Trawl Surveyl (Percentiles)",
                # scale = "column",
                # margins = c(60,100,40,20),
                # grid_color = "white",
                # grid_width = 0.000005,
