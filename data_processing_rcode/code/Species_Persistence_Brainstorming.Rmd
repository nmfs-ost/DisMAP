...
data_3$Bin <- factor(data_3$Bin, levels = c('Absent', 'Rare', 'Less Common', 'Common', 'Abundant', 'Highly Abundant'))
...
p4 <- heatmaply(mat_4,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 4: NEFSC Spring Bottom Trawl (Haul Proportion)",
                hide_colorbar = FALSE,
                label_names = c("Species", "Year", "Percentile"),
                fontsize_row = 5, 
                fontsize_col = 5,
...
data_7$Bin <- factor(data_7$Bin, levels = c('Absent', 'Rare', 'Less Common', 'Common', 'Abundant', 'Highly Abundant'))
...
mat_8 <- as.matrix(data_8)
rownames(mat_8) <- mat_8[,1]
mat_8 <- as.data.frame(mat_8) %>% 
  dplyr::select(-Species) %>% 
  mutate_if(is.character, as.numeric)
...
p8 <- heatmaply(mat_8,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 8: Aleutian Islands (Haul Proportion)",
                hide_colorbar = FALSE,
                label_names = c("Species", "Year", "Percentile"),
                fontsize_row = 5, 
                fontsize_col = 5,
                labCol = colnames(mat_8),
                labRow = rownames(mat_8),
                heatmap_layers = theme(axis.line = element_blank())
                )
