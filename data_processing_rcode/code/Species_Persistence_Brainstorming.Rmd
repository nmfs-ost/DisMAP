...
```{r scratch 2, eval=FALSE}

#### HAVING TROUBLE SAVING THE GGPLOTS IN A KNITTABLE FORMAT ####
...
for(v in surveys) {
  ...
  # Display the plot in the knitted .Rmd document
  print(plot_list) # This will ensure the plots are displayed inline in the .Rmd
...
- Emerging: species colonized the region but never went extinct
- Leaving: species went extinct from the region but were never observed to have colonized
- Sporadic: experienced at least one colonization and extinction
- Stable: not experienced a colonization nor an extinction
...
data_ce_ap$Trend <- ifelse(data_ce_ap$col == 1 & data_ce_ap$ext == 0, "Emerging",
                           ifelse(data_ce_ap$col == 0 & data_ce_ap$ext == 1, "Leaving",
                                  ifelse(data_ce_ap$Absence == 0, "Stable", 
                                         ifelse(data_ce_ap$col > 2 & data_ce_ap$ext > 2, "Sporadic", 0))))
