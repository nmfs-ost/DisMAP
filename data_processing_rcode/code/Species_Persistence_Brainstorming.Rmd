...
data_rank <- data_update %>%
  ...
  group_by(region, survey, year, common, spp) %>%
...
data_haul <- preshaul %>% 
  select(region, survey, spp, common, year, pres) %>% 
  dplyr::rename(Region = region,
                Species = spp,
                Common = common,
                SurveyName = survey,
...
data_1 <- data_full %>%
  filter(SurveyName == "West Coast Bottom Trawl Annual") %>% 
  ungroup() %>% 
...
p1 <- heatmaply(mat_1,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 1: West Coast Bottom Trawl Annual",
                # scale = "column",
...
p2 <- heatmaply(mat_2,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 2: West Coast Bottom Trawl Annual (Percentiles)",
                hide_colorbar = FALSE,
...
p3 <- heatmaply(mat_3,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 3: NEFSC Spring Bottom Trawl",
                hide_colorbar = FALSE,
                label_names = c("Species", "Year", "Presence"),
                fontsize_row = 5, 
                fontsize_col = 5,
...
data_4 <- data_full %>%
  filter(SurveyName == "NEFSC Spring Bottom Trawl") %>% 
  ungroup() %>% 
  dplyr::select(-Region, -Common, -SurveyName, -WTCPUE, -Presence, -Bin) %>% 
  pivot_wider(names_from = Year, values_from = Percentile)
...
p4 <- heatmaply(mat_4,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 4: NEFSC Spring Bottom Trawl (Percentiles)",
                hide_colorbar = FALSE,
                # branches_lwd = 0.1,
                label_names = c("Species", "Year", "Percentile"),
                fontsize_row = 5, 
...
p5 <- heatmaply(mat_5,
                ...
                xlab = "", 
                ylab = "",
                main = "Ex 5: Aleutian Islands Bottom Trawl Survey",
                hide_colorbar = FALSE,
                label_names = c("Species", "Year", "Presence"),
                fontsize_row = 5, 
                fontsize_col = 5,
