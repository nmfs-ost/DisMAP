...
# ------------------------------------------ #
 ## THIS FUNCTION IS USED IN THE Fix_Taxon_QAQC_data.R script
# ------------#
# Explanation
# ------------#
...
clean_taxa <- function(taxon_list, input_survey = "NA", save = F, output = NA, fishbase=TRUE){
  ...
  print(paste0("Returned ", nrow(output_df), " taxa and dropped ",n_dropped,". Misspelled taxa: ",n_misspell,"; No alphia id found: ",nrow(missing_alphaid),"; Non-marine taxa: ", nrow(missing_salt_fish)," All taxa assessed =", nrow(output_df) + n_dropped  == length(taxon_list)))
  e_time <- Sys.time()
  print(s_time-e_time)
  return(output_df)
