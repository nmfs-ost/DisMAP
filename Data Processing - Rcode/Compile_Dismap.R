## ---- DISMAP 3/17/2023
## updated thru 2022 survey data for all regions except SEUS (which is thru 2021)
...
# 1. compile_Dismap.R script - this script
...
# 4. R directory - folder containing scripts used in the making of this script
...
explode0 <- function(x, by=c("region")){
  ...
  setorder(x, sampleid, stratum, year, lat, lon, stratumarea, depth)
  ...
  # pull out these location related columns and sort by sample_id and year
  x.loc <- x[,list(sampleid, year, stratum, stratumarea, lat, lon, depth)]
  setkey(x.loc, sampleid, year)
  
  # attatch all spp to all locations
  x.skele <- x.loc[,list(spp=u.spp, common=u.cmmn), by=eval(colnames(x.loc))]
  setkey(x.skele, sampleid, year, spp)
  x.skele <- unique(x.skele)
  setcolorder(x.skele, c("sampleid","year","spp", "common", "stratum", "stratumarea","lat","lon","depth"))
  
  # pull in multiple observations of the same species 
  x.spp.dat <- x[,list(sampleid, year, spp, wtcpue)]
  setkey(x.spp.dat, sampleid, year, spp)
...
if(isTRUE(WRITE_MASTER_DAT)){
  save(ai, ebs, gmex, goa, neus_fall, neus_spring, seusFALL, seusSPRING, seusSUMMER, tax, wcann, wctri, file = here("data_clean", "individual-regions.rds"))
}
if(isTRUE(WRITE_MASTER_DAT)){
  save(ai_fltr, ebs_fltr, gmex_fltr, goa_fltr, neus_fall_fltr, neus_spring_fltr, seusFALL_fltr, seusSPRING_fltr, seusSUMMER_fltr, tax, wcann_fltr, wctri_fltr, file = here("data_clean", "individual-regions-fltr.rds"))
}
